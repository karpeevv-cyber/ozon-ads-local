...
from strategy_map import get_strategy, load_strategy_map, upsert_strategy
from ui_data import (
...
from ui_helpers import (
    ...
    get_ui_state_entry,
    save_ui_state_entry,
    load_campaign_reco_map,
    save_campaign_reco_map,
...
RECO_CACHE_PATH = "campaign_reco.csv"
...
if prev_params and prev_params != current_params:
    for k in ["rows_csv", "daily_rows", "ads_daily_by_campaign", "products_by_campaign_id", "by_day_sku"]:
        st.session_state.pop(k, None)
    st.session_state.cache_loaded = False
st.session_state._last_params = current_params

recalc_recommendations = st.sidebar.checkbox("recalc recommendations", value=False)

batch_size = 15
include_products = True

# Load cached recommendations once per session
if "campaign_reco_map" not in st.session_state:
    st.session_state.campaign_reco_map = load_campaign_reco_map(RECO_CACHE_PATH)
...
if go:
    try:
        ...
        with st.spinner("Loading data..."):
            ...
            by_sku, by_day, by_day_sku = seller_analytics_sku_day(
                str(date_from),
                str(date_to),
                limit=1000,
            ...
            ads_daily_rows, ads_daily_by_campaign = fetch_ads_daily_totals(
                token,
                str(date_from),
                str(date_to),
                running_ids,
            ...
            if recalc_recommendations:
                st.session_state.reco_requested = True
                st.session_state.reco_blocked_reason = ""
                last_ts = float(st.session_state.get("reco_last_ts", 0))
                now_ts = time.time()
                if now_ts - last_ts < 60:
                    st.session_state.reco_running = False
                    st.session_state.reco_queue = []
                    st.session_state.reco_index = 0
                    st.session_state.reco_blocked_reason = "cooldown"
                    st.warning("Recalc allowed once per minute.")
                else:
                    st.session_state.reco_last_ts = now_ts
                    ids = [str(c.get("id")) for c in running]
                    st.session_state.reco_queue = ids
                    st.session_state.reco_index = 0
                    st.session_state.reco_running = True
                    base_reco_map = load_campaign_reco_map(RECO_CACHE_PATH)
                    st.session_state.campaign_reco_map = dict(base_reco_map)
                    st.session_state.reco_errors = 0
                    st.session_state.reco_last_exception = ""
                    st.session_state.reco_current_id = None
                    st.session_state.reco_last_step_ts = None
                    st.session_state.go_stage = "reco_started"

                    # Synchronous recommendation calculation (fast path)
                    for i, cid in enumerate(ids):
                        try:
                            st.session_state.reco_current_id = cid
                            if ads_daily_by_campaign:
                                camp_daily = build_campaign_daily_rows_cached(
                                    ...
                                    seller_by_day_sku=st.session_state.get("by_day_sku") or {},
                                    ads_daily_by_campaign=ads_daily_by_campaign,
                                    target_drr=target_drr,
                                    items=products_by_campaign_id.get(cid, []) or [],
                                )
                            else:
                                camp_daily = build_campaign_daily_rows(
                                    token=token,
                                    campaign_id=cid,
                                    date_from=str(st.session_state.get("date_from", date_from)),
                                    date_to=str(st.session_state.get("date_to", date_to)),
                            ...
                            strat = control_drr_strategy(df_camp, target_drr=target_drr)

                            bid_current = None
                            items = products_by_campaign_id.get(cid, []) or []
                            skus = [str(x.get("sku")) for x in items if x.get("sku") is not None]
                            if len(skus) == 1:
                                raw_bid = items[0].get("bid")
                                try:
                                    bid_current = int(
                                        float(str(raw_bid).strip().replace(" ", "").replace(",", "."))
                                    ) / 1_000_000
                                except Exception:
                                    bid_current = None

                            bid_next = strat.get("bid_next")
                            if bid_next is None and strat.get("status") == "NO_ACTION_OK":
                                bid_next = bid_current

                            views_min_day_need_1_3 = strat.get("views_min_day_need_1_3")
                            views_now_day = strat.get("views_day_avg", 0.0) or 0.0

                            stop_by_visibility = (
                                views_min_day_need_1_3 is not None
                                and views_now_day < views_min_day_need_1_3
                                and bid_next is not None
                                and bid_current is not None
                                and bid_next < bid_current
                            )
                            stop_by_floor = (
                                strat.get("bid_floor") is not None
                                and strat.get("cpc_econ_max") is not None
                                and strat.get("bid_floor") > strat.get("cpc_econ_max")
                            )
                            stop_by_low_visibility = (
                                views_min_day_need_1_3 is not None
                                and views_now_day * 2 < views_min_day_need_1_3
                            )

                            reco = "3" if (stop_by_visibility or stop_by_floor or stop_by_low_visibility) else "1"
                            st.session_state.campaign_reco_map[cid] = reco
                        except Exception as e:
                            st.session_state.campaign_reco_map[cid] = "?"
                            st.session_state.reco_errors = (st.session_state.get("reco_errors") or 0) + 1
                            st.session_state.reco_last_exception = repr(e)
                        finally:
                            st.session_state.reco_last_step_ts = time.time()
                            st.session_state.reco_index = i + 1

                    st.session_state.reco_running = False
                    st.session_state.reco_current_id = None
                    st.session_state.go_stage = "reco_done"
                    save_campaign_reco_map(RECO_CACHE_PATH, st.session_state.campaign_reco_map)
        st.session_state.last_error = ""
        if not recalc_recommendations:
            st.session_state.go_stage = "done"
        if not recalc_recommendations:
            st.session_state.reco_requested = False
    except Exception as e:
...
campaign_reco_map = st.session_state.get("campaign_reco_map", {})
...
# Incremental recommendation calculation
...
if not rows_csv:
    ...
    if st.session_state.get("reco_running"):
        st.session_state.reco_running = False
...
with tab1:
    ...
    if daily_rows:
        ...
        if "day" in df_daily_raw.columns and "total_revenue" in df_daily_raw.columns:
            ...
            chart_df = df_chart.set_index("day_dt")[["total_revenue"]]
...
with tab2:
    ...
    df_campaigns["recommended_strategy"] = df_campaigns["campaign_id"].astype(str).map(campaign_reco_map).fillna("?")
    if "strategy" not in df_campaigns.columns:
        def _strategy_for_row(r):
            sku = str(r.get("sku", "") or "").strip()
            if sku in {"", "None", "several"}:
                return ""
            return get_strategy(str(r.get("campaign_id")), sku) or ""
    ...
    if ads_daily_by_campaign and by_day_sku:
        def _cpc_econ_range_for_campaign(r):
            cid = str(r.get("campaign_id"))
            camp_daily = build_campaign_daily_rows_cached(
                campaign_id=cid,
                date_from=str(st.session_state.get("date_from", date_from)),
            ...
            df_camp = pd.DataFrame(camp_daily)
            strat = control_drr_strategy(df_camp, target_drr=target_drr)
            cpc_econ_min = strat.get("cpc_econ_min")
            cpc_econ = strat.get("cpc_econ")
            cpc_econ_max = strat.get("cpc_econ_max")
        ...
        df_campaigns["cpc_econ_range"] = df_campaigns.apply(_cpc_econ_range_for_campaign, axis=1)
    else:
    ...
    st.dataframe(
        style_median_table(df_campaigns_view, metrics_campaigns, band_pct=BAND_PCT),
        width="stretch",
        hide_index=True,
...
with tab3:
    ...
    if not picked_campaign_id:
        ...
        strat = control_drr_strategy(df_camp_daily_raw, target_drr=target_drr)
        ...
        if bid_sku_for_detail and picked_campaign_id:
            current_strategy_display = get_strategy(str(picked_campaign_id), str(bid_sku_for_detail))
        if current_strategy_display not in {"1", "2", "3"}:
            current_strategy_display = "1"
        strategy_labels = {"1": "Контроль DRR", "3": "STOP"}
        strategy_label = strategy_labels.get(current_strategy_display)
        if strategy_label:
            strategy_text = f"Стратегия: {current_strategy_display} ({strategy_label})"
        else:
            strategy_text = f"Стратегия: {current_strategy_display}"

        st.markdown("### Параметры стратегии")
        cpc_econ = strat.get("cpc_econ")
        bid_floor = strat.get("bid_floor")
        cpc_econ_max = strat.get("cpc_econ_max")
        bid_next_candidates = [v for v in [cpc_econ, bid_floor] if v is not None]
        bid_next = min(bid_next_candidates) if bid_next_candidates else None
        if bid_next is not None and cpc_econ_max is not None:
            bid_next = min(bid_next, cpc_econ_max)
        if bid_next is None and strat.get("status") == "NO_ACTION_OK":
            bid_next = current_bid_rub

        views_min_day_need_1_3 = strat.get("views_min_day_need_1_3")
        lines = [
            strategy_text,
            f"Текущий bid: {fmt_rub_1(current_bid_rub) if current_bid_rub is not None else '—'}",
            f"Рекомендованный bid: {fmt_rub_1(bid_next) if bid_next is not None else '—'}",
            f"Статус: {str(strat.get('status', '—'))}",
            "===",
            f"CPC econ: {fmt_rub_1(strat.get('cpc_econ')) if strat.get('cpc_econ') is not None else '—'}",
            f"CPC econ min: {fmt_rub_1(strat.get('cpc_econ_min')) if strat.get('cpc_econ_min') is not None else '—'}",
            f"CPC econ max: {fmt_rub_1(strat.get('cpc_econ_max')) if strat.get('cpc_econ_max') is not None else '—'}",
            f"Bid floor: {fmt_rub_1(strat.get('bid_floor')) if strat.get('bid_floor') is not None else '—'}",
            "===",
            f"DRR 7d: {fmt_pct_1((strat.get('drr_short') or 0.0) * 100.0) if strat.get('drr_short') is not None else '—'}",
            f"views_long 21d: {fmt_int_space(strat.get('views_long', 0))}",
            f"clicks_long 21d: {fmt_int_space(strat.get('clicks_long', 0))}",
            f"orders_long 21d: {fmt_int_space(strat.get('orders_long', 0))}",
            f"revenue_short 7d: {fmt_rub_space(strat.get('revenue_short', 0.0))}",
            f"CR_long 21d: {fmt_pct_1((strat.get('cr') or 0.0) * 100.0)}",
            f"views_day_avg_long 21d: {fmt_int_space(strat.get('views_day_avg_long', 0.0))}",
            f"views_min_day_for_floor: {fmt_int_space(strat.get('views_min_day_for_floor', 0.0))}",
            f"views_min_day_need_1/3: {fmt_int_space(views_min_day_need_1_3) if views_min_day_need_1_3 is not None else '—'}",
            f"Views_now_day: {fmt_int_space(strat.get('views_day_avg', 0.0))}",
        ]
        st.text("\n".join(lines))

        if strat.get("status") in {"LOW_DATA_LONG", "LOW_DATA_SHORT"}:
            st.caption(f"LOW_DATA причина: {strat.get('missing', '—')}")

        stop_reco = (
            current_strategy_display != "3"
            and (
                (
                    views_min_day_need_1_3 is not None
                    and (strat.get("views_day_avg", 0.0) or 0.0) < views_min_day_need_1_3
                    and bid_next is not None
                    and current_bid_rub is not None
                    and bid_next < current_bid_rub
                )
                or (
                    views_min_day_need_1_3 is not None
                    and (strat.get("views_day_avg", 0.0) or 0.0) * 2 < views_min_day_need_1_3
                )
                or (
                    strat.get("bid_floor") is not None
                    and strat.get("cpc_econ_max") is not None
        ...
        if stop_reco:
            st.warning("Рекомендация: стратегия №3 — STOP")
