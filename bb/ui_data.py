...
from ui_formatting import to_num_series
...
def control_drr_strategy(
    df_camp_daily_raw: pd.DataFrame,
    target_drr: float,
    min_orders_week: float = 2.0,
    drr_abs_tolerance: float = 0.05,
    floor_percentile: float = 0.3,
):
    """
    Strategy 1: Control DRR.
    Returns dict with status and recommended bid components.
    """
    ...
    today = date.today()
    start_long = today - timedelta(days=21)
    start_short = today - timedelta(days=7)

    df_long_all = df[(df["day"] >= start_long) & (df["day"] <= today)].copy()
    df_short = df[(df["day"] >= start_short) & (df["day"] <= today)].copy()

    views_long = float(pd.to_numeric(df_long_all.get("views", 0), errors="coerce").fillna(0).sum())
    clicks_long = float(pd.to_numeric(df_long_all.get("clicks", 0), errors="coerce").fillna(0).sum())
    orders_long = float(pd.to_numeric(df_long_all.get("ordered_units", 0), errors="coerce").fillna(0).sum())
    days_long = int(df_long_all["day"].nunique()) if "day" in df_long_all.columns else 0
    if days_long <= 0:
        days_long = 21
    views_day_avg_long = views_long / days_long

    if views_long == 0 or clicks_long < 30 or orders_long < 2:
        missing = []
        if views_long == 0:
            missing.append("views_long==0")
        if clicks_long < 30:
            missing.append("clicks_long<30")
        if orders_long < 2:
            missing.append("orders_long<2")
        return {
            "status": "LOW_DATA_LONG",
            "missing": ", ".join(missing),
            "views_long": views_long,
            "clicks_long": clicks_long,
            "orders_long": orders_long,
            "views_day_avg_long": views_day_avg_long,
        }

    revenue_short = float(pd.to_numeric(df_short.get("total_revenue", 0), errors="coerce").fillna(0).sum())
    spend_short = float(pd.to_numeric(df_short.get("money_spent", 0), errors="coerce").fillna(0).sum())
    if revenue_short <= 0:
        return {
            "status": "LOW_DATA_SHORT",
            "missing": "revenue_short<=0",
    ...
    vor = (orders_long / views_long) if views_long > 0 else 0.0
    if vor == 0:
        return {
            "status": "NO_CONV",
            "views_long": views_long,
    ...
    views_min_week = min_orders_week / vor
    views_min_day_for_floor = views_min_week / 7.0

    # Step B: Visibility floor
    df_long = df_long_all[df_long_all["day"] < today].copy()  # completed days only
    df_long = df_long[pd.to_numeric(df_long.get("views", 0), errors="coerce").fillna(0) > 0]
    df_long = df_long[pd.to_numeric(df_long.get("clicks", 0), errors="coerce").fillna(0) >= 2]

    if not df_long.empty:
        def _eff_cpc(row):
            clicks = float(row.get("clicks", 0) or 0)
            spend = float(row.get("money_spent", 0) or 0)
            if clicks > 0:
                return spend / clicks
            return float(row.get("click_price", 0) or 0)

        df_long["eff_cpc"] = df_long.apply(_eff_cpc, axis=1)
        df_long["views_num"] = pd.to_numeric(df_long.get("views", 0), errors="coerce").fillna(0)
        s = df_long[df_long["views_num"] >= views_min_day_for_floor]["eff_cpc"].dropna().tolist()
    else:
        s = []
    ...
    status_floor = None
    if not s:
        status_floor = "NO_FLOOR_DATA"
    else:
        bid_floor = float(pd.Series(s).quantile(floor_percentile))

    # Step C: Economic CPC
    revenue_long = float(pd.to_numeric(df_long_all.get("total_revenue", 0), errors="coerce").fillna(0).sum())
    order_value = (revenue_long / orders_long) if orders_long > 0 else 0.0
    cr = (orders_long / clicks_long) if clicks_long > 0 else 0.0
    ...
    # Step D: No-action zone
    drr_short = (spend_short / revenue_short) if revenue_short > 0 else None
    days_short = df_short["day"].nunique() if "day" in df_short.columns else 0
    days_short = days_short if days_short > 0 else 7
    views_short = float(pd.to_numeric(df_short.get("views", 0), errors="coerce").fillna(0).sum())
    views_day_avg = views_short / days_short

    if drr_short is not None and drr_min <= drr_short <= drr_max and views_day_avg >= views_min_day_for_floor:
        return {
            "status": "NO_ACTION_OK",
            "bid_next": None,
            "bid_floor": bid_floor,
            ...
            "views_min_day_for_floor": views_min_day_for_floor,
            "views_day_avg": views_day_avg,
            "drr_short": drr_short,
    ...
    if bid_floor is not None:
        bid_base = max(bid_base, bid_floor)

    # Recommendation: Strategy #3 STOP (text-only)
    # Views_now_day = views_day_avg; Views_need_day = min_orders_week / vor / 3
    views_min_day_need_1_3 = (min_orders_week / vor) / 3.0 if vor > 0 else 0.0

    return {
        "status": "OK" if status_floor is None else status_floor,
        "bid_next": bid_base,
        "bid_floor": bid_floor,
        "cpc_econ": cpc_econ,
        "cpc_econ_min": cpc_econ_min,
        ...
        "drr_max": drr_max,
        "views_day_avg_long": views_day_avg_long,
        "views_min_day_need_1_3": views_min_day_need_1_3,
    }
