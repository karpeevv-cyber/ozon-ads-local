...
from trend_external import load_external_suggestion_signals
...
from trend_sources import build_date_span, load_catalog, load_query_signals, load_sales_history


QUERY_NOISE_TOKENS = {
    "buy", "benefits", "recipe", "youtube", "ozon", "wb", "wildberries", "price",
    "how", "near", "starbucks", "set", "shop", "kupit", "tsena", "otzyvy",
    "купить", "цена", "цены", "отзывы", "ютуб", "вайлдберриз", "озон", "польза",
    "вред", "рецепт", "похудения", "своими", "руками", "для",
}
...
def _title_tokens(title: str) -> list[str]:
    raw = re.findall(r"[A-Za-zА-Яа-я0-9]+", _safe_text(title).lower())
...
def _normalize_niche_tokens(tokens: list[str]) -> list[str]:
    ...
    for token in tokens:
        ...
        if not token or token in QUERY_NOISE_TOKENS:
            continue
        if token.endswith(("ами", "ями")) and len(token) > 5:
            token = token[:-3]
        elif token.endswith(("ов", "ев", "ом", "ем", "ах", "ях", "ый", "ая", "ое", "ие", "ий")) and len(token) > 4:
            token = token[:-2]
        elif token.endswith(("ы", "и", "а", "я")) and len(token) > 4:
            token = token[:-1]
